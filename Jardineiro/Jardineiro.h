...
public:
    ...
    [[nodiscard]] bool podeMover() const { return movimentosRestantes; }
    ...
    [[nodiscard]] bool podePlantar() const;

    void registaPlantacao();

    [[nodiscard]] bool podeColher() const;
    ...
    [[nodiscard]] bool getEstaNoJardim() const { return estaNoJardim; }

    [[nodiscard]] int getPosLin() const { return posLin; }
    [[nodiscard]] int getPosCol() const { return posCol; }

    // devolve a ferramenta ativa (na mão) do jardineiro
    [[nodiscard]] Ferramenta *getFerramentaAtiva() const { return ferramentaAtiva; }

    // lista as ferramentas no inventario (vetor) do jardineiro
    [[nodiscard]] vector<string> listarFerramentas() const;

private:
    int posLin = -1, posCol = -1;
    bool estaNoJardim = false;
    vector<Ferramenta *> inventario; // inventario de ferramentas do jardineiro
